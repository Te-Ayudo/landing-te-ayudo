...
const industries = [
  { name: "Todos", color: "#ff770f", value: "all" },
  { name: "Hogar", color: "#5ccdcc", value: "Hogar" },
  { name: "Vehículos", color: "#ff770f", value: "Vehículos" },
  { name: "Salud", color: "#5ccdcc", value: "Salud" },
  { name: "Belleza", color: "#ff770f", value: "Belleza" },
];
...
const Partners = () => {
  ...
  const filteredLogos = activeIndustry === "all" ? partnerLogos : partnerLogos.filter((logo) => logo.industry === activeIndustry);
    ...
    <section id="partners" className="py-16 bg-[#ffffff]">
      <div className="container mx-auto px-4 max-w-7xl">
        ...
        <div className="mb-6 relative">
          <div className="flex justify-between items-center mb-2">
            ...
            <div className="flex space-x-2 md:hidden">
              <button onClick={() => scrollFilters("left")} className="p-2 rounded-full bg-gray-100 hover:bg-gray-200 transition-colors" aria-label={language === "es" ? "Desplazar a la izquierda" : "Scroll left"}>
                <ChevronLeft className="h-5 w-5" />
              </button>
              <button onClick={() => scrollFilters("right")} className="p-2 rounded-full bg-gray-100 hover:bg-gray-200 transition-colors" aria-label={language === "es" ? "Desplazar a la derecha" : "Scroll right"}>
                <ChevronRight className="h-5 w-5" />
              </button>
            </div>
          </div>

          <div ref={filtersRef} className="flex overflow-x-auto md:flex-wrap scrollbar-hide pb-2 gap-2" style={{ scrollbarWidth: "none", msOverflowStyle: "none" }}>
            {industries.map((industry) => (
              <button
                key={industry.value}
                onClick={() => setActiveIndustry(industry.value)}
                className={`px-4 py-2 rounded-full text-sm font-medium whitespace-nowrap flex-shrink-0 transition-colors ${activeIndustry === industry.value ? "text-white" : "bg-gray-100 text-gray-700 hover:bg-gray-200"}`}
                style={{
                  backgroundColor: activeIndustry === industry.value ? industry.color : undefined,
                }}
              >
                {industry.name}
              </button>
        ...
        <div className="flex justify-end mb-6">
          <button onClick={() => setShowAllLogos(!showAllLogos)} className="text-sm font-medium text-[#ff770f] bg-[#ff770f]/10 px-4 py-2 rounded-md hover:bg-[#ff770f]/20 transition-colors">
        ...
        {!showAllLogos && (
          <div className="relative">
            <div ref={carouselRef} className="overflow-hidden rounded-xl border border-gray-100 shadow-sm bg-white p-6">
              <div className="relative" style={{ height: "250px" }}>
                {" "}
                {/* Fixed height container */}
                {organizedLogos.map((page, pageIndex) => (
                  <div key={`page-${pageIndex}`} className={`absolute top-0 left-0 w-full transition-opacity duration-500 ease-in-out ${pageIndex === currentSlide ? "opacity-100 z-10" : "opacity-0 z-0"}`}>
                    {page.map((row, rowIndex) => (
                      <div key={`row-${pageIndex}-${rowIndex}`} className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-6 gap-4 w-full mb-6">
                        {row.map((partner, partnerIndex) => (
                          <div key={`${partner.name}-${pageIndex}-${rowIndex}-${partnerIndex}`} className="flex flex-col items-center justify-center p-3 hover:scale-110 transition-transform duration-300 cursor-pointer">
                            <div className="bg-gray-50 p-3 rounded-full mb-2 hover:bg-[#fff8f0] transition-colors flex items-center justify-center" style={{ width: "70px", height: "70px" }}>
                              <img src={partner.logo} alt={`Logo de ${partner.name}`} className="h-12 w-12 object-contain" loading="lazy" />
                            </div>
            ...
            {totalSlides > 1 && (
              <>
                <div className="absolute inset-y-0 left-0 flex items-center">
                  <button onClick={goToPrevSlide} className="p-2 rounded-full bg-white shadow-md text-gray-700 hover:text-[#ff770f] transition-colors -ml-3" aria-label={language === "es" ? "Anterior" : "Previous"}>
                    <ChevronLeft className="h-5 w-5" />
                  </button>
                </div>
                <div className="absolute inset-y-0 right-0 flex items-center">
                  <button onClick={goToNextSlide} className="p-2 rounded-full bg-white shadow-md text-gray-700 hover:text-[#ff770f] transition-colors -mr-3" aria-label={language === "es" ? "Siguiente" : "Next"}>
                    <ChevronRight className="h-5 w-5" />
                  </button>
                </div>
                ...
                <div className="flex justify-center mt-4 space-x-2">
                  {Array.from({ length: totalSlides }).map((_, index) => (
                    <button key={`indicator-${index}`} onClick={() => setCurrentSlide(index)} className={`w-2.5 h-2.5 rounded-full transition-colors ${currentSlide === index ? "bg-[#ff770f]" : "bg-gray-300 hover:bg-gray-400"}`} aria-label={`Ir a página ${index + 1}`} />
                  ))}
                </div>
              </>
            )}
        ...
        {showAllLogos && (
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-4 md:gap-6">
            {filteredLogos.map((partner, index) => (
              <div key={`grid-${partner.name}-${index}`} className="flex flex-col items-center justify-center p-4 bg-white rounded-lg shadow-sm border border-gray-100 hover:shadow-md transition-all hover:border-[#ff770f]/30 hover:scale-105 group cursor-pointer">
                <div className="bg-gray-50 p-3 rounded-full mb-3 group-hover:bg-[#fff8f0] transition-colors flex items-center justify-center" style={{ width: "80px", height: "80px" }}>
                  <img src={partner.logo} alt={`Logo de ${partner.name}`} className="h-14 w-14 object-contain" loading="lazy" />
                </div>
